...
def integrate_arc(image, center, measure_arc, r):
    ...
    num1 = np.round(512*(measure_arc[3] + measure_arc[2])/90).astype(np.uint8)
    angles_right = np.linspace(-measure_arc[3], measure_arc[2], num1) 
    x_right = np.array(center[0] + r * np.cos(angles_right))
    y_right = np.array(center[1] - r * np.sin(angles_right))    
    num2 = np.round(512*(measure_arc[0] + measure_arc[1])/90).astype(np.uint8)
    angles_left = np.linspace(180 - measure_arc[0], 180 + measure_arc[1], num2)
    x_left = np.array(center[0] + r * np.cos(angles_left))
    y_left = np.array(center[1] - r * np.sin(angles_left))
    ...
    if np.any(x >= rows - 1) or np.any(y >= cols - 1) or np.any(x <= 1) or np.any(y <= 1):
        ...
        return 0
    # points_1 = bilinear_interpolate(image, x_left, y_left)
    #points_2 = bilinear_interpolate(image, x_right, y_right)
    ...
    return np.sum(points) / (2 * np.pi * r)


def gauss_kernel(x, mu=0, sigma=100):
...
def find_iris(image, y, x, radius):
    ...
    #print("{}/{}: {}".format(idx, len(result), result[idx]))

    return int(n_delta_r[idx]), max(results), x, y
...
for name in glob.glob("D:/CASIA-IrisV4-Interval/**/*[0-9].jpg", recursive=True):
    ...
    r = int(circle[0][0][2]) 
    """
    x = np.arange(int(circle[0][0][0] - 5), int(circle[0][0][0] + 5))
    y = np.arange(int(circle[0][0][1] - 5), int(circle[0][0][1] + 5))
    xx, yy = np.meshgrid(x, y)
    xx, yy = np.round(xx/2).astype(np.uint8), np.round(yy/2).astype(np.uint8)
    #center = np.transpose([np.tile(x, len(y)), np.repeat(y, len(x))])
    #print (xx, yy)
    ...
    filename = 'C:/Users/user/Downloads/iris/hough_test/image_{}.jpg'.format(it)
    filepath = os.path.join('images_', filename)
    print(filepath, name)
